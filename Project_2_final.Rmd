...
```{r proportions_list, message=F, fig.width = 6.5, fig.height = 6.5}
...
```{r complex_roc}
# library(pROC)
complex_predictions <- predict(glmnet.fit, validate, type = "prob")[,"Yes"]
...
plot(complex.roc, col = "blue",print.thres="best", lwd = 2, main = "ROC Curve")


#abline(v = best_threshold, col = "red", lty = 2)
legend("bottomright", legend = c("Complex Model"
...
complex.preds <- factor(ifelse(complex_predictions > threshold, "Yes", "No"),
                    levels = c("No", "Yes"))
...
Some of this performance has been lower than hoped

```{r pca_vs_complex}
set.seed(123)

# Exclude the response variable "fracture" and any other unwanted columns
train_exclude <- c(1:3, which(names(training) == "fracture"))
train_select <- training[, -train_exclude]

# Perform PCA on the selected features

train_encoded <- data.frame(sapply(train_select, function(col) if(is.factor(col)) as.numeric(col) else col))
pc.train.result<-prcomp(train_encoded, scale.=T)
pc_train_data <- data.frame(pc.train.result$x, fracture = training$fracture)

#scree plot for optimization
ideal_variance <- 0.9
eigenvals <- pc.train.result$sdev^2
...
# TRAIN THE PCA
pc_select <- data.frame(pc.train.result$x[, 1:8], fracture = pc_train_data$fracture)



plot_ggpairs <- ggpairs(data = pc_select,  title = "PCA #8 Pairs Plot",
        aes(color = fracture, fill = fracture)) +
  scale_color_manual(values = class_colors) +
  scale_fill_manual(values = class_colors  )

print(plot_ggpairs, progress=F, message=F, warning=F)
...
# TRANSFORM VALIDLATE SET FOR PCA
pca.validation_select <- validate[, -train_exclude]  # Exclude the same columns as in training
pca.validation_encoded <- data.frame(sapply(pca.validation_select, function(col) if(is.factor(col)) as.numeric(col) else col))
pca.validation_transformed <- predict(pc.train.result, newdata = pca.validation_encoded)

# Predict using the complex_model_pca trained on training data
validate_predictions <- predict(complex_model_pca, pca.validation_transformed, type = "prob")[,"Yes"]


# use ROC to help tune the threshold
pca.roc <- roc(response = validate$fracture, predictor = validate_predictions,levels = c("No", "Yes"))
plot(pca.roc, print.thres = "best")



threshold <- 0.23

# Predict on the PCA-processed validation data
pca.validate_predictions <- predict(complex_model_pca, pca.validation_transformed, type = "prob")[,"Yes"]

# Create predicted labels based on the threshold
complex_pca_preds <- factor(ifelse(pca.validate_predictions > threshold, "Yes", "No"),
                            levels = c("No", "Yes"))

# Set factor levels to match the actual labels in validate
complex_pca_preds <- factor(complex_pca_preds, levels = levels(validate$fracture))

# Calculate confusion matrix
confusion_complex_pca <- confusionMatrix(data = complex_pca_preds,
                                         reference = validate$fracture,
                                         positive = "Yes")

# Display the confusion matrix
confusion_complex_pca
```
...
# For use when comparing competing models
qda.roc <-roc_avg
```
...
```{r model_score_confusion_matrix}
knn1.probabilities <- knn1.probabilities %>%
  mutate(class = ifelse(Predicted_prob > 0.24, "Yes", "No")) %>%
  mutate(class = factor(class))
conf1 <- confusionMatrix(table(knn1.probabilities$class,dftrain$fracture), positive = "Yes")
...
cat("Model Scores\n\n")
cat("############ KNN ##############                ############ QDA ##############                ######### Complex Model #########\n")                # k = 15 (previous feature selection) #\n")
conf1t <- capture.output(conf1)
conf2t <- capture.output(conf2)
conf3t <- capture.output(confusion_complex)
conf4t <- capture.output(conf.simple) 
conf5t <- capture.output(confusion_complex_pca)
...
combined_text2 <- paste(str_pad(conf4t,width=40)
                       , str_pad(conf5t,width=40)
                       , sep = "\t")

# Displaying the combined text output
cat(combined_text, sep = "\n")  
cat("\n\n########### Simple #############             ############ PCA ##############\n")
cat(combined_text2, sep = "\n")

```

```{r model_score_AUROC, fig.width=7, fig.height=5}
# All 4 Models
plot(knnfinal.roc, col = "#990943", print.thres="best",lwd = 2, main = "ROC Curves Comparison")
lines(qda.roc, col = "#08A338", lwd = 2)
lines(complex.roc, col = "#3908A3",    lwd = 2)
lines(roc.simple, col = "salmon", lwd = 2)
lines(pca.roc, col = "turquoise", lwd = 2)
legend("bottomright", legend = 
  c(
    paste0("KNN (", round(auc(knnfinal.roc),3), " AUROC)"),
    paste0("QDA (", round(auc(qda.roc),3), " AUROC)"),
    paste0("Complex MLR (", round(auc(complex.roc),3), " AUROC)"),
    paste0("Simple MLR (", round(auc(roc.simple),3), " AUROC)"),
    paste0("PCA MLR (", round(auc(pca.roc),3), " AUROC)")
  ),
  col = c("#990943", "#08A338","#3908A3","salmon","turquoise"), lwd = 2)
```

# Conclusion


We have found that the best combination of scores can be found within our KNN model.  No model performs the best with every single metric here, but KNN did lead on both Negative Predictive Value and AUROC scores, while also scoring near the top in every metric used in this analysis.  Its strong performance at all decision thresholds is also evident in the above ROC Curve comparison.
